...
class V74ExportController extends Controller
{
    use HandlesOtApprovalAccess;

    public function download(
        DownloadV74Request $request,
        V74ExportService $exporter,
        ...
        try {
            $export = $exporter->export($scope, $date, $request->validated('cycle'), $submittedOn);
            ...
            return $this->failed(
                $request,
                $dateValue,
                'ไม่สามารถสร้างไฟล์ V74 ได้ในขณะนี้ กรุณาลองใหม่อีกครั้ง',
            );
        }
    ...
    private function failed(
        ...
        return redirect()
            ->route('ot-approval.home', is_string($date) ? ['date' => $date] : [])
            ->with('toast_fallback', $message)
